...
import robot_lab.tasks.lite3.mdp as mdp
from robot_lab.assets.lite3 import (
    LITE3_BASE_LINK_NAME,
    LITE3_CFG,
    LITE3_FOOT_LINK_NAME,
    LITE3_JOINT_NAMES,
)
from robot_lab.tasks.lite3.mdp.terrains import TERRAIN_CFG

JOINT_NAMES = LITE3_JOINT_NAMES

BASE_LINK_NAME = LITE3_BASE_LINK_NAME
FOOT_LINK_NAME = LITE3_FOOT_LINK_NAME

# ---------------------------------------------------------------------------
# 身体高度参考值与地形缩放基准
#
# LITE3_BODY_HEIGHT_REF 取自 money12532/Lite3_RL_Project
#   (rl_training/.../assets/deeprobotics.py 的 init_state.pos.z = 0.375，
#    与部署端 lite3_test_policy_runner_onnx.cpp 的默认高度一致)。
# GO2_BODY_HEIGHT_REF 取 RobotLab Go2 任务的 BASE_HEIGHT_TARGET = 0.38
#   (source/robot_lab/robot_lab/tasks/go2/env_cfg.py)。
#
# 地形难度按身体高度等比例缩放：
#     terrain_scale = LITE3_BODY_HEIGHT_REF / GO2_BODY_HEIGHT_REF
# 缩放后的具体数值见 lite3/mdp/terrains.py 顶部说明。
# ---------------------------------------------------------------------------
LITE3_BODY_HEIGHT_REF = 0.375
GO2_BODY_HEIGHT_REF = 0.38
TERRAIN_HEIGHT_SCALE = LITE3_BODY_HEIGHT_REF / GO2_BODY_HEIGHT_REF  # ~ 0.98684
...
@configclass
class Lite3SceneCfg(InteractiveSceneCfg):
    """Configuration for the terrain scene with the Lite3 robot."""
    ...
    robot: ArticulationCfg = LITE3_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")
...
class ActionsCfg:
    ...
    # action scale 取自 money12532/Lite3_RL_Project：
    #   rough_env_cfg.py: actions.joint_pos.scale = {".*_HipX_joint": 0.125,
    #                                               "^(?!.*_HipX_joint).*": 0.25}
    # 与部署端 lite3_control_parameters.cpp 的 action_scale_robot 一致。
    # clip=(-5, 5) 同样取自参考仓（防止 actor mean 发散导致关节静默饱和）。
...
class RewardsCfg:
    ...
    Reward design inherited from the RobotLab Go2 task unchanged. The only
    difference is the target height of base_height_l2 / feet_regulation, which
    uses the Lite3 value 0.55 m from money12532/Lite3_RL_Project
    (rough_env_cfg.py: rewards.base_height_l2.params["target_height"]).
    """
    ...
    hip_pos_penalty_l1 = RewTerm(
        ...
        params={
            ...
            "asset_cfg": SceneEntityCfg("robot", joint_names=".*_HipX_joint"),
    ...
    joint_pos_penalty_l1 = RewTerm(
        ...
        params={
            "command_name": "base_velocity",
            "asset_cfg": SceneEntityCfg("robot", joint_names=".*_(HipY|Knee)_joint"),
            "stand_still_scale": 1.0,
...
@configclass
class Lite3EnvCfg(ManagerBasedRLEnvCfg):
    """Merged configuration for the Lite3 robot on rough terrain."""

    # Scene settings
    scene: Lite3SceneCfg = Lite3SceneCfg(num_envs=16384, env_spacing=0.5)
    # Basic settings
